...
import com.cortxt.app.mmccore.Utils.MMCCallbacks;
import com.cortxt.app.mmcutility.ContentProvider.ContentValuesGeneratorOld;
import com.cortxt.app.mmcutility.ContentProvider.ProviderOld;
...
import com.cortxt.app.mmccore.Services.MMCPhoneStateListener;
...
import com.cortxt.app.mmccore.Services.TrackingManager;
...
import com.cortxt.app.mmccore.UtilsOld.CellHistory;
...
public class MMCService extends Service {
	...
	private MMCPhoneStateListener phoneStateListener;
	private static GpsManagerOld gpsManager, netLocationManager;
	private MMCServiceIntentDispatcher intentDispatcher;
	private MMCIntentHandler intentHandler;
	private MMCWebSocket webSocketManager;
	private VQManager mVQManager;
	private static TrackingManager trackingManager;
	private MMCCallbacks mmcCallbacks; // communication with extension library which implements premium features
	...
	public void onCreate() {
		...
		try {
			mmcCallbacks = new MMCCallbacks(this);
			...
			intentDispatcher = new MMCServiceIntentDispatcher(this);
			intentHandler = new MMCIntentHandler(this, dataMonitorStats);
			//killMMCZombies();
			...
			phoneStateListener = new MMCPhoneStateListener(this, mPhoneState);
	...
	public int onStartCommand (Intent intent, int flags, int startId) {
        ...
        if ((intent != null) && intent.hasExtra (MMCIntentHandler.GCM_MESSAGE_EXTRA)) {
            ...
            String msg = intent.getStringExtra(MMCIntentHandler.GCM_MESSAGE_EXTRA);
	...
	public void setAlarmManager ()// get a Calendar object with current time
	{
		...
		Intent intent2 = new Intent(MMCIntentHandler.ACTION_ALARM_3HOUR);
	...
	public void set15MinuteAlarmManager (int interval, Integer appscan_sec)
	{
		...
		if(interval > 0) {
		    ...
		    Intent intent = new Intent(MMCIntentHandler.ACTION_ALARM_15MINUTE);
			...
			intent = new Intent(MMCIntentHandler.ACTION_ALARM_SCANAPPS);
			...
			if (useHeartbeat)
			{
				...
				Intent intent = new Intent(MMCIntentHandler.ACTION_ALARM_SCANAPPS);
				PendingIntent alarm = PendingIntent.getBroadcast(this,0,intent, PendingIntent.FLAG_CANCEL_CURRENT);
	...
	public void stop15MinAlarmManager ()
	{
		...
		Intent intent2 = new Intent(MMCIntentHandler.ACTION_ALARM_15MINUTE);
		PendingIntent alarm2 = PendingIntent.getBroadcast( 
				this, 
				0, 
				intent2, 
	...
	public void processNewFilteredLocation(Location location, int satellites) {
		...
		getDBProvider(this).insert(TablesEnumOld.LOCATIONS.getContentUri(), values);
	}
	...
	private void checkNotificationExpiry ()
	{
		...
		{
			notificationManager.cancel(MMCPhoneStateListener.MMC_DROPPED_NOTIFICATION);
		}
	}
		...
		if (systemAlertPermissionValue == 0) {
			...
			b.setOnCancelListener(new DialogInterface.OnCancelListener() {
				...
				public void onCancel(DialogInterface dialog) {
					svcPanel.dismiss();
					svcPanel = null;
					MMCPhoneStateListener.closedServicePanel = true;
				}
			});
	...
	public MMCPhoneStateListener getPhoneStateListener(){
	...
	public void makeApplicationForeground(boolean bForeground, String message) {
		...
		if (DeviceInfoOld.getPlatform() != 3 && !android.os.Build.BRAND.toLowerCase().contains("blackberry")) //  && message != null && message.length() > 0)
		{
			...
			Intent intent = new Intent(MMCIntentHandler.MMC_SERVICE_FOREGROUND);
			intent.putExtra(MMCIntentHandler.MMC_FOREGROUND, bForeground);
			if (message == null)
				intent.putExtra(MMCIntentHandler.MMC_FOREGROUND_MESSAGE, title);
			else
				intent.putExtra(MMCIntentHandler.MMC_FOREGROUND_MESSAGE, message);
