...
public class EventManager {
	...
	public EventManager(MMCService context){
...
//		String evtid = eventCouple.getStartEvent().getUri().getLastPathSegment();
	...
	public void signalSnapshot (EventObj event)
	{
		...
		try {
			...
			MMCSignal signal = context.getPhoneState().getLastMMCSignal();
			...
			MMCCellLocation cell = context.getPhoneStateListener().getLastCellLocation();
			if (cell != null)
			{
...
//	}
	...
	public void setActiveTestComplete (EventType evtType)
	{
		...
		MMCLogger.logToFile(MMCLogger.Level.DEBUG, TAG, "setActiveTestComplete", "test=" + evtType.getIntValue() + ", latch=" + testingLatch);
