...
import com.cortxt.app.mmcutility.ContentProvider.ContentValuesGeneratorOld;
import com.cortxt.app.mmcutility.ContentProvider.TablesEnumOld;
import com.cortxt.app.mmccore.MMCService;
import com.cortxt.app.mmccore.Utils.RestCommManager;
...
import com.cortxt.app.mmcutility.Reporters.LocalStorageReporter.LocalStorageReporter.Events;
 ...
 * </ol>
 */
public class MMCPhoneStateListener extends PhoneStateListener {
	...
	public static final String TAG = MMCPhoneStateListener.class.getSimpleName();
...
//	private int lastKnownCallState;
	 ...
	 */
	public MMCPhoneStateListener(MMCService owner, PhoneState phonestate) {
		this.owner = owner;
...
//	private String lastCellString = "";
	...
	private String lastCellString = "";
	private MMCSignal prevMMCSignal = null;
	...
	public void onCellLocationChanged(CellLocation location) {
		...
		try {
			...
			processNewCellLocation(new MMCCellLocation(location));
	...
	public void onDataConnectionStateChanged(int state, int networkType){
		...
		MMCSignal signal = mPhoneState.getLastMMCSignal();
		if (signal != null)
		{
	...
	public void onSignalStrengthsChanged(SignalStrength signalStrength) {
		...
		try {
			...
			{
				MMCSignal mmcSignal = new MMCSignal(signalStrength);
...
//				}
	...
	class VerifyConnectTask extends TimerTask {
		...
		public void run() {
			...
			MMCLogger.logToFile(MMCLogger.Level.WTF, TAG, "VerifyConnectTask", "call connected=" + mPhoneState.isCallConnected() + " event=" + targetEventCouple);
			...
			{	
				MMCLogger.logToFile(MMCLogger.Level.DEBUG, TAG, "onCallStateChanged", "off hook too short: " + (mPhoneState.disconnectTime - mPhoneState.offhookTime));
	...
	class DisconnectTimerTask extends TimerTask {
		...
		public void run() {
			...
			MMCLogger.logToFile(MMCLogger.Level.WTF, TAG, "DisconnectTimerTask", "call connected=" + mPhoneState.isCallConnected() + " event=" + targetEventCouple);
			...
			if(mPhoneState.isCallConnected() || mPhoneState.isCallDialing() || mPhoneState.isCallRinging())
			{
				...
				{
					MMCLogger.logToFile(MMCLogger.Level.WTF, TAG, "DisconnectTimerTask", "call dropped heuristic=" + mPhoneState.heurCause);
...
//					setCallWaiting(true);
		...
		Intent intent = new Intent(MMCIntentHandler.PHONE_CALL_CONNECT);
	...
	public void onCallStateChanged(int state, String incomingNumber) {
		...
		try
		{
			...
			switch (state){
				...
				intent = new Intent(MMCIntentHandler.PHONE_CALL_DISCONNECT);
				owner.sendBroadcast(intent);
	...
	public void onServiceStateChanged(ServiceState serviceState) {
		...
		if (mPhoneState.isRoaming() != wasRoaming) {
			...
			if(mPhoneState.isRoaming()) {
				...
				Intent intent = new Intent(MMCIntentHandler.ROAMING_ON);
				owner.sendBroadcast(intent);
			}
			else {
				Intent intent = new Intent(MMCIntentHandler.ROAMING_OFF);
				owner.sendBroadcast(intent);
			}
		...
		if(PhoneState.ActiveConnection(owner) == 12) {
			Intent intent = new Intent(MMCIntentHandler.WIMAX_STATE_CHANGE);
			owner.sendBroadcast(intent);
		}
	...
	public void onServiceMenu ( String _timestamp, String values, String name) {
		...
		if (!closedServicePanel && owner.isMonitoringActive())
			MMCService.updateSvcPanel(owner, values, name);
	}
			...
			if (name.equals("BASIC")) {
				if (!mPhoneState.prevSvcValues.equals(values)) {
					...
					if (bCellChanged == true) {
						MMCCellLocation cell = getLastCellLocation();
						if (cell != null) {
	...
	*  Precise Information about phone-call-connect states, if available from logcat or other priviledged means
	...
	public void popupDropped (final EventType droptype, final int rating, final int evtId)
	{
		...
		owner.handler.post(new Runnable() {
			...
			public void run() {
				String message = "";
				int icon;
				icon = com.cortxt.app.mmccore.R.drawable.ic_stat_dropped;
				String title = "";
				String msg = "";

				// server can specify whether a confirmation can be invoked on a low rated potentially-dropped call
				int allowConfirm = PreferenceManager.getDefaultSharedPreferences(owner).getInt(PreferenceKeys.Miscellaneous.ALLOW_CONFIRMATION, 5);
				String noConfirm = (owner.getResources().getString(com.cortxt.app.mmccore.R.string.NO_CONFIRMATION));
	...
	public void processNewCellLocation(MMCCellLocation cellLoc) throws InterruptedException {
		...
		try
		{
			...
			Intent intent = new Intent(MMCIntentHandler.HANDOFF);
			owner.sendBroadcast(intent);
		}
		catch (Exception e)
		{
	...
	public void processNewMMCSignal(MMCSignal signal)  {
	...
	public MMCCellLocation getLastCellLocation(){
		if (mPhoneState.lastKnownMMCCellLocation != null)
		...
		if (cellLoc != null)
		{
			...
			MMCCellLocation mmcCell = new MMCCellLocation(cellLoc);
			try {
