package com.cortxt.app.mmccore.Services.Intents;
...
import com.cortxt.app.mmcutility.ContentProvider.TablesEnumOld;
...
import com.cortxt.app.mmccore.UtilsOld.DataMonitorStats;
import com.cortxt.app.mmcutility.Utils.DeviceInfoOld;
import com.cortxt.app.mmccore.Services.Events.EventResponse;
 ...
 *
 */
public class MMCIntentHandler extends BroadcastReceiver {
	public static final String TAG = MMCIntentHandler.class.getSimpleName();
...
//	public static final String ACTION_NETWORK_LOCATION_UPDATE = "com.cortxt.app.MMC.intent.action.ACTION_NETWORK_LOCATION_UPDATE";
	 ...
	 * constructor
	 */
	public MMCIntentHandler(MMCService owner, DataMonitorStats dataMonitorStats){
		this.owner = owner;	
...
//			owner.trackAccessPoints();
			...
			try {
				...
				MMCSignal mmcSignal = new MMCSignal();
...
//    				PreferenceKeys.Miscellaneous.SURVEY_COMMAND,).commit();
			...
			if(reportManager.manualTransitEvent != null) {
				...
				reportManager.getDBProvider().delete(TablesEnumOld.LOCATIONS.getContentUri(), "timestamp > ? And accuracy = 3", new String[]{ String.valueOf(reportManager.manualTransitEvent.getEventTimestamp())});
				reportManager.manualTransitEvent = null;
			}
		}
...
//			owner.getEventManager().unstageEvent(manulaPlottingEvent); //does not upload
			...
			if(reportManager.manualPlottingEvent != null)
			{
				...
				reportManager.getDBProvider().delete(TablesEnumOld.LOCATIONS.getContentUri(), "timestamp > ? And accuracy < 0", new String[]{ String.valueOf(reportManager.manualPlottingEvent.getEventTimestamp())});
				reportManager.manualPlottingEvent = null;
...
//			}
	...
	public IntentFilter declareIntentFilters() {
		IntentFilter intentFilter = new IntentFilter(MMCIntentHandler.UPDATE_ACTION);
		intentFilter.addAction(MMCIntentHandler.START_TRACKING_ACTION);
		intentFilter.addAction(MMCIntentHandler.TRACK_REMOTE);
		intentFilter.addAction(MMCIntentHandler.STOP_TRACKING_ACTION);
		intentFilter.addAction(MMCIntentHandler.SPEED_TEST);
		intentFilter.addAction(MMCIntentHandler.RUN_WEBSOCKET);
		...
		intentFilter.addAction(MMCIntentHandler.ACTION_SPEEDTEST_RESULT);
		intentFilter.addAction(MMCIntentHandler.ACTION_SPEEDTEST_ERROR);
		intentFilter.addAction(MMCIntentHandler.ACTION_SPEEDTEST_COMPLETE);
		intentFilter.addAction(MMCIntentHandler.ACTION_SPEEDTEST_CANCELLED);

		intentFilter.addAction(MMCIntentHandler.ACTION_WEBTEST_RESULT);
		intentFilter.addAction(MMCIntentHandler.ACTION_WEBTEST_ERROR);
		intentFilter.addAction(MMCIntentHandler.ACTION_WEBTEST_COMPLETE);
		intentFilter.addAction(MMCIntentHandler.ACTION_WEBTEST_CANCELLED);

		//do not add filter if sms test permissions aren't all allowed
		if (PreferenceKeys.getSMSPermissionsAllowed(owner, true))
			intentFilter.addAction(MMCIntentHandler.SMS_TEST);

		intentFilter.addAction(MMCIntentHandler.SMS_DELIVERED);
		intentFilter.addAction(MMCIntentHandler.LATENCY_TEST);
		intentFilter.addAction(MMCIntentHandler.ACTION_STOP_SPEEDTEST);
		intentFilter.addAction(MMCIntentHandler.RESTART_MMC_SERVICE);
		intentFilter.addAction(MMCIntentHandler.COLDRESET_ACTION);
		intentFilter.addAction(Intent.ACTION_BATTERY_CHANGED);
		intentFilter.addAction(Intent.ACTION_POWER_CONNECTED);
		intentFilter.addAction(Intent.ACTION_POWER_DISCONNECTED);
		intentFilter.addAction(Intent.ACTION_SCREEN_OFF);
		intentFilter.addAction(Intent.ACTION_SCREEN_ON);
		intentFilter.addAction(MMCIntentHandler.COMMAND);
		intentFilter.addAction(MMCIntentHandler.SURVEY);
		//intentFilter.addAction(Intent.ACTION_VIEW);
		//intentFilter.addAction("android.intent.PHONE_STATE");
		intentFilter.addAction(WifiManager.NETWORK_STATE_CHANGED_ACTION);
		intentFilter.addAction(MMCIntentHandler.ACTION_ALARM_MINUTE);
		intentFilter.addAction(MMCIntentHandler.ACTION_TRACKING_5MINUTE);
		intentFilter.addAction(MMCIntentHandler.ACTION_TRACKING_1MINUTE);
		intentFilter.addAction(MMCIntentHandler.ACTION_ALARM_3HOUR);
		intentFilter.addAction(MMCIntentHandler.ACTION_ALARM_15MINUTE);
		intentFilter.addAction(MMCIntentHandler.ACTION_ALARM_SCANAPPS);
		intentFilter.addAction(MMCIntentHandler.EMAIL_CSV);
		intentFilter.addAction(MMCIntentHandler.GPS_STATE_OFF);
		intentFilter.addAction(MMCIntentHandler.GPS_STATE_ON);
		intentFilter.addAction(MMCIntentHandler.PHONE_CALL_CONNECT);
		intentFilter.addAction(MMCIntentHandler.PHONE_CALL_DISCONNECT);
		intentFilter.addAction(MMCIntentHandler.HANDOFF);
		intentFilter.addAction(MMCIntentHandler.SMS_SENT);
		intentFilter.addAction(MMCIntentHandler.SMS_RECEIVED);
		intentFilter.addAction(Intent.ACTION_SEND);
		intentFilter.addAction(MMCIntentHandler.MANUAL_PLOTTING_START);
		intentFilter.addAction(MMCIntentHandler.MANUAL_PLOTTING_END);
		intentFilter.addAction(MMCIntentHandler.MANUAL_PLOTTING_CANCEL);
		intentFilter.addAction(MMCIntentHandler.MANUAL_TRANSIT_START);
		intentFilter.addAction(MMCIntentHandler.MANUAL_TRANSIT_END);
		intentFilter.addAction(MMCIntentHandler.MANUAL_TRANSIT_CANCEL);
		intentFilter.addAction(BluetoothDevice.ACTION_ACL_CONNECTED);
		intentFilter.addAction(BluetoothDevice.ACTION_ACL_DISCONNECT_REQUESTED);
		intentFilter.addAction(BluetoothDevice.ACTION_ACL_DISCONNECTED);

		intentFilter.addAction(Intent.ACTION_HEADSET_PLUG);
		intentFilter.addAction(MMCIntentHandler.ROAMING_ON);
		intentFilter.addAction(MMCIntentHandler.ROAMING_OFF);
...
//		intentFilter.addAction(MMCIntentHandlerOld.ACTION_NETWORK_LOCATION_UPDATE);
		...
		intentFilter.addAction(MMCIntentHandler.VIEWING_SIGNAL);
...
//		intentFilter.addAction("com.android.internal.telephony.dataconnectiontracker.intent_set_fail_data_setup_counter");
		...
		intentFilter.addAction(MMCIntentHandler.ACTIVE_TEST);
		intentFilter.addAction(MMCIntentHandler.ACTION_STOP_VIDEOTEST);
		intentFilter.addAction(MMCIntentHandler.GCM_MESSAGE);
		...
		intentFilter.addAction (MMCIntentHandler.ACTION_RADIOLOG_DISCONNECT);
		intentFilter.addAction (MMCIntentHandler.ACTION_RADIOLOG_CONNECT);
		intentFilter.addAction (MMCIntentHandler.ACTION_RADIOLOG_NEIGHBORS);
		intentFilter.addAction (MMCIntentHandler.ACTION_RADIOLOG_SERVICEMODE);
		intentFilter.addAction (MMCIntentHandler.ACTION_RADIOLOG_SERVICEMENU);
		intentFilter.addAction (MMCIntentHandler.ACTION_MMCSYS_VERSION);
